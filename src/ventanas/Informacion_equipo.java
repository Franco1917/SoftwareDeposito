...
public class Informacion_equipo extends javax.swing.JFrame {
   ...
   int ID_cliente_update= 0, IDequipo = 0, IDClienteUpdate= 0;
   ...
   String user= "", nom_cliente= "";
    ...
    public Informacion_equipo() {
        initComponents();
    user= Login.user;
    IDequipo = Informacion_Clientes.id_equipo;
    IDClienteUpdate = Gestionar_Clientes.ID_cliente_update;
    
    
    try{
        Connection cn = Conexion.conectar() ;
        PreparedStatement pst = cn.prepareStatement(
        "select nomblre_clientes from clientes ehere id_cliente = '" +IDClienteUpdate +"'"  );
    ...
    setTitle("Equipo cliente " + nom_cliente);
    setResizable(false);
    ...
    private void initComponents() {
        ...
        getContentPane().add(jToggleButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(490, 380, 120, 50));
    ...
    private void jToggleButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jToggleButton1ActionPerformed
     ...
     estatus = combo_status.getSelectedItem().toString();
     
     modelo= txt_modelo.getText().trim();
     num_serie= txt_num1.getText().trim();
     observaciones = txt_obvservaciones.getText();
     if (modelo.equals("")){
         txt_modelo.setBackground(Color.red);
         validacion++;
     }
     if (num_serie.equals("")){
         txt_num1.setBackground(Color.red);
         validacion++;
     }
     if (observaciones.equals("")){
         txt_obvservaciones.setText("sin obvservaciones");
     ...
     if(validacion==0){
         try{
             Connection cn = Conexion.conectar();
             PreparedStatement pst = cn.prepareStatement(
             "update  equipos  set tipo_equipo =?, marca=?, modelo= ?,num_serie =?, observaciones=?, estatus=?, ultima_modificacion=? where ID_equipo = '" + IDequipo +"'");
